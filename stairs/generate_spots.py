...
generate_spots.py
=================
stairs/data/spots.js を読み込んで stairs/spot/S*.html を一括生成するスクリプト。

使い方：
    1. このスクリプトを stepjourney/stairs/ フォルダに置く
    2. ターミナルで stairs/ フォルダに移動する
         cd path/to/stepjourney/stairs
    3. 実行する
         python3 generate_spots.py
    4. stairs/spot/ フォルダに S001.html〜S***.html が生成（上書き）される
    5. 生成されたファイルをそのまま GitHub にアップロードする

spots.js を更新したら、このスクリプトを再実行して spot/ フォルダを丸ごと上書きアップロードしてください。
"""
...
# ── パス設定 ──────────────────────────────────────────────
SPOTS_JS   = os.path.join(os.path.dirname(__file__), 'data', 'spots.js')
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), 'spot')
# ─────────────────────────────────────────────────────────

EXTS = ['jpg', 'JPG', 'png']

def load_spots(js_path):
    """spots.js から JSON 部分だけを抽出してパースする"""
    ...
    if not m:
        raise ValueError('spots.js から spotsData が見つかりませんでした')
...
def generate_html(spot):
    ...
    elev_text = str(ev) if (ev != '' and ev is not None) else '情報なし'
...
<div class="section">
  ...
  <div class="info-row">
    <div class="info-label">推定標高差</div>
    <div class="info-value">{elev_text}</div>
  </div>
  <div class="info-row">
...
def main():
    print(f'spots.js を読み込み中: {SPOTS_JS}')
    spots = load_spots(SPOTS_JS)
