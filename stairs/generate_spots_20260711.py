...
def photo_section(names, spot_name):
    ...
    main = f'<div class="photo-main">{img_tag(names[0], "メイン写真", "main-img", style="width:100%;height:auto;display:block;")}</div>'
    if len(names) > 1:
        thumbs_inner = ''.join(
...
CSS = '''
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .photo-main { background:var(--ai-mid); width:100%; overflow:hidden; display:flex; align-items:center; justify-content:center; border-radius:0 0 13px 13px; border:1px solid var(--stone-bd); border-top:none; }
  ...
  .photo-thumbs { display:flex; gap:6px; padding:8px 12px; background:#ede9e2; overflow-x:auto; border:1px solid var(--stone-bd); border-bottom:none; border-radius:13px 13px 0 0; }
